...
for mnd in datelist:
    ...
    for rolle in roller_list:  # man kan ha flere roller
        ...
        df_rolle_mnd = (
            df_rolle_mnd.groupby(["dato_mnd", "Rolle", "Omraade", "gender_pred"])
            .size()
...
df_mnd_total.rename(
    columns={
        "Total": "antall_totalt",
        "sum_kvinner": "antall_kvinner",
        "sum_ukjent": "antall_ukjent",
    },
    inplace=True,
)
...
df_rolle_omraade = (
    df_rolle_omraadeT.pivot(
        index=["dato_mnd", "Rolle", "Omraade"], columns="gender_pred", values="Antall"
    )
    .reset_index()
    .fillna(0)
)
df_rolle_omraade["Total"] = (
    df_rolle_omraade["female"] + df_rolle_omraade["male"] + df_rolle_omraade["unknown"]
)
df_rolle_omraade.rename(
    columns={
        "Total": "antall_totalt",
        "female": "antall_kvinner",
        "unknown": "antall_ukjent",
    },
    inplace=True,
)
...
df_rolle = (
    df_rolle_omraadeT.groupby(["dato_mnd", "Rolle", "gender_pred"])[["Antall"]]
    .sum()
    .reset_index()
)
df_rolle = (
    df_rolle.pivot(index=["dato_mnd", "Rolle"], columns="gender_pred", values="Antall")
    .reset_index()
    .fillna(0)
)
...
write_to_BQ(
    client=bq_client,
    table_name="omraade_rolle_stats",
    dframe=df_rolle_omraade,
    dataset=DATASET,
    disp="WRITE_TRUNCATE",
)
write_to_BQ(
    client=bq_client,
    table_name="omraade_stats",
    dframe=df_mnd_total,
    dataset=DATASET,
    disp="WRITE_TRUNCATE",
)
